...
func deepCopyDatabase(source Database) Database {
    copiedInfo := deepCopyDBinfo(source.Info)
    copiedMatrix := deepCopyMatrix(source.Data)
    copiedDB := Database{
        Info: copiedInfo,
        Data: &copiedMatrix,
    }
...
func deepCopyDBinfo(source DBinfo) DBinfo {
    copiedInfo := DBinfo{
        Num:        source.Num,
        Row_length: source.Row_length,
...
func deepCopyMatrix(source *Matrix) Matrix {
    // Create a deep copy of the Matrix struct
    copiedMatrix := Matrix{
        Rows: source.Rows,
        Cols: source.Cols,
        Data: make([]C.Elem, len(source.Data)),
    }

    for i, elem := range source.Data {
        copiedMatrix.Data[i] = elem
    }
...
func (m *Matrix) Dim() {
	...
	SERVER_DB_DIMENSION = strconv.FormatUint(m.Rows, 10) + "-by-" + strconv.FormatUint(m.Cols, 10)
}
