...
func (pi *DoublePIR) RecoverFile(i uint64, batch_index uint64, offline Msg, query Msg,
	answer Msg, shared State, client State, p Params, info DBinfo, originalSize int) uint64 {
	originalSize = 100
	H2 := offline.Data[0]
	h1 := answer.Data[0].RowsDeepCopy(0, answer.Data[0].Rows) // deep copy whole matrix 
	secret1 := client.Data[0]
